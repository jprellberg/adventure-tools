...
* Books: every book listed in `books.json` (see 2.3).
...
Two kinds are extracted from the text of books and adventures: Tables (every captioned table, plus the standalone tables file) and Book Rules (named `entries` blocks one or two levels below a chapter that have prose of their own; books only), each with a breadcrumb path to where it sits. Adventures, and the chapters and deeper blocks of books, are not entities of their own; adventure metadata is only used to give the source tag its full title. Data that only feeds 5etools' generators and tools (loot and name generators, the encounter builder, homebrew maker tables, life-event tables, item-type descriptions, fluff/lore and images files) is not loaded.
...
Routing: dioxus-router managing application paths (/, /entities/:kind/:source/:name, /books/:source/:..section).
