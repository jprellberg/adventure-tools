...
A round button with three vertical dots to the right of the search bar opens the filter menu: a card border appears around the search bar, the button and a filter area below them (scrolling when it gets tall), and the page on either side of the card is dimmed slightly but stays visible. The button is tinted amber while the filters differ from the default of the selected ruleset (every kind on, every source on except the other edition's core rulebooks), so that it shows that entities are being filtered out. ESC or a click anywhere outside the card closes the menu. The area holds:
